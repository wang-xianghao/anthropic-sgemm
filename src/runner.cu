...
void run_sgemm_coalesced(int M, int N, int K, float alpha, float *A, float *B,
                     float beta, float *C) {
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32, 32);
...
void run_sgemm_shared(int M, int N, int K, float alpha, float *A, float *B,
                     float beta, float *C) {
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32 * 32);
  sgemm_shared<32>
...
void run_sgemm_1d_blocktiling(int M, int N, int K, float alpha, float *A, float *B,
                     float beta, float *C) {
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 1:
    ...
    break;
  case 2:
    run_sgemm_coalesced(M, N, K, alpha, A, B, beta, C);
    break;
  case 3:
    run_sgemm_shared(M, N, K, alpha, A, B, beta, C);
    break;
  case 4:
